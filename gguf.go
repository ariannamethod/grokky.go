...
func parseMetadata(kv map[string]interface{}) GGUFMetadata {
	...
	// MoE metadata (Grok-style)
	if v, ok := kv[arch+".expert_count"]; ok {
		meta.NumExperts = toInt(v)
	}
	if v, ok := kv[arch+".expert_used_count"]; ok {
		meta.NumExpertsPerTok = toInt(v)
	}
	if v, ok := kv["grok.shared_expert"]; ok {
		if b, ok := v.(bool); ok {
			meta.HasSharedExpert = b
		}
	}
	if v, ok := kv["grok.use_gelu"]; ok {
		if b, ok := v.(bool); ok {
			meta.UseGELU = b
		}
	}
	if v, ok := kv["grok.double_prenorm"]; ok {
		if b, ok := v.(bool); ok {
			meta.DoublePrenorm = b
		}
	}
	if v, ok := kv["grok.attn_clamp"]; ok {
		meta.AttnClamp = toFloat32(v)
	}
